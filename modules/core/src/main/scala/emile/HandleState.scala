...
/** Phantom type hierarchy for handle lifecycle state.
  *
  * These types are used as type parameters on handle types to track their state at compile time.
  * This prevents operations on closed handles from even compiling.
  *
  * ==Usage==
  *
  * {{{
...
/** Phantom type indicating a closed handle.
  *
  * Handles in the `Closed` state cannot be used for any operations. The only valid operation is
  * checking the type for tracking purposes.
  *
  * Note: libuv close is asynchronous, so `Closed` means "close initiated" not necessarily "close
  * completed". The handle memory is freed when the close callback fires.
  */
