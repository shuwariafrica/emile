...
// scalafix:off DisableSyntax.null, DisableSyntax.asInstanceOf, DisableSyntax.while; libuv FFI loop management
...
/** Handle to a libuv event loop.
  *
  * Manages the lifecycle of all handles and drives I/O operations. The loop is the central
  * structure in libuv, processing events and dispatching callbacks.
  */
...
object Loop:
  ...
  /** Obtain the default global event loop.
    *
    * The default loop is a global singleton managed by libuv. It should NOT be closed by the
    * application.
    *
  ...
  /** Create a new event loop with libuv defaults.
    *
    * The caller is responsible for closing the loop when done. This is the most efficient path when
    * no configuration overrides are needed.
    *
  ...
  /** Apply configuration overrides to an existing loop.
    ...
    *
    * Note: Loop configuration should normally be done before the first call to `run`. Some options
    * may not take effect if applied later.
    */
  ...
  extension (loop: Loop)
    ...
    /** Stop the event loop.
      *
      * Causes `run` to return as soon as possible. This will happen no sooner than the next loop
      * iteration. If called before blocking for I/O, the loop won't block for I/O on this
      * iteration.
      */
    def stop: Unit =
      LibUV.uv_stop(loop)
    ...
    /** Get current cached timestamp in milliseconds.
      *
      * The timestamp is cached at the start of the event loop tick. Use `updateTime` to refresh it
      * if needed.
      *
    ...
    /** Update the loop's cached timestamp.
      *
      * libuv caches the current time at the start of the event loop tick. This avoids excessive
      * syscalls. Call this to force a time update.
      */
    def updateTime: Unit =
    ...
    /** Walk all handles and initiate close on each.
      *
      * This is the first step of async loop cleanup. After calling this, continue running the loop
      * until `isAlive` returns false, then call `close` to release the loop memory.
      *
    ...
    /** Close all handles and then close the loop (synchronous).
      *
      * This walks all handles in the loop, closes them, runs the loop until all close callbacks
      * have fired, then closes the loop itself.
      *
      * WARNING: This does blocking uv_run calls which may not be compatible with async effect
      * systems. For cats-effect, prefer using the integrated loop resource which handles cleanup
      * asynchronously.
      *
    ...
    def closeDrain: Either[EmileError, Unit] =
      val defaultLoop = LibUV.uv_default_loop()
      if loop == defaultLoop then Right(())
      else
        LibUV.uv_walk(loop, walkCloseCallback, null.asInstanceOf[Ptr[Byte]])
    ...
    /** Get accumulated idle time in nanoseconds.
      *
      * Requires `LoopConfig.metricsEnabled = true` to be set during loop creation. If metrics were
      * not enabled, this returns 0.
      *
  ...
  private val walkCloseCallback: LibUV.WalkCB = (handle: Ptr[Byte], _: Ptr[Byte]) =>
    if LibUV.uv_is_closing(handle) == 0 then LibUV.uv_close(handle, Handle.nullCloseCallback)
end Loop
