...
/** Native platform extensions for IP address and socket address types.
  *
  * These extension methods provide zero-copy conversion between emile-ipa types and native sockaddr
  * structures used by POSIX networking APIs.
  *
  * ==Usage==
  *
  * {{{
...
extension (addr: Ipv4Address)
  ...
  /** Fill a sockaddr_in structure with this address.
    *
    * @param sockaddr Pointer to a sockaddr_in structure to fill
...
extension (addr: Ipv6Address)
  /** Fill a 16-byte buffer with the address in network order.
    *
    * @param buf Pointer to a 16-byte buffer
    */
  def fillNetworkOrder(buf: Ptr[Byte]): Unit =
    ...
    while i < 8 do
      ...
      i += 1
  end fillNetworkOrder

  /** Fill a sockaddr_in6 structure with this address.
    *
    * @param sockaddr Pointer to a sockaddr_in6 structure to fill
    * @param port The port number
    * @param flowInfo IPv6 flow information
    * @param scopeId IPv6 scope identifier
    */
  def fillSockAddrIn6(
...
extension (addr: SocketAddress)
  /** Allocate and fill a sockaddr structure for this address.
    ...
    *
    * @return Pointer to the allocated sockaddr structure (sockaddr_in or sockaddr_in6)
    */
  ...
  /** Get the size of the sockaddr structure for this address type.
    *
    * @return Size in bytes (16 for IPv4, 28 for IPv6)
    */
...
/** Parse a SocketAddress from a sockaddr structure.
  *
  * @param sockaddr Pointer to a sockaddr structure
  * @return Either an error or the parsed SocketAddress
  */
